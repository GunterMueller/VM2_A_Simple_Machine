...
FROM Symbol IMPORT symbol, scopeType, symbolScope, symbolString, numLocals;
...
(* obsolete.      
PROCEDURE genRelOp(tc:tokenClass);
VAR label1: label;

    PROCEDURE genRelOpIf(b:BOOLEAN);
    VAR label2:label;
    BEGIN
        label2 := newLabel();
        writeBool(b);
...
BEGIN
    ...
    writeOp(Minus);
    CASE tc OF
        Equal:      writeCondBranch(Equal, label1);
                    genRelOpIf(FALSE);
    |   NotEqual:   writeCondBranch(Equal, label1);
                    genRelOpIf(TRUE);
    |   Greater:    writeCondBranch(Greater, label1);
                    genRelOpIf(FALSE);   
    |   GreaterEqual:writeCondBranch(Less, label1);
                     genRelOpIf(TRUE);
    |   Less:       writeCondBranch(Less, label1);
                    genRelOpIf(FALSE);           
    |   LessEqual:  writeCondBranch(Greater, label1);
                    genRelOpIf(TRUE);
    ELSE
        MyTerminal.fatal("genRelOp: unknown relation type");
    END;
END genRelOp;
*)
